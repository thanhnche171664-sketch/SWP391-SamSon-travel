...
<!--            <a href="AddBooking.jsp"><i class="fas fa-plus-circle"></i><span>Booking mới</span></a>-->
        ...
        <section class="content">
            ...
            <div class="content-header">
                ...
                <c:if test="${not empty param.returnUrl}">
                    <div class="mb-3">
                        <a href="${param.returnUrl}" class="btn btn-outline-secondary btn-sm" onclick="returnToCustomerInfo(); return false;">
                            <i class="fas fa-arrow-left me-1"></i> Quay lại Customer Info
                        </a>
                    </div>
                </c:if>
            ...
            <div class="cart-summary" id="cartSummary" style="display: none;">
                ...
                <button class="btn-checkout" onclick="confirmOfflineServicePayment()">
    ...
    <aside class="mini-cart" id="miniCart" style="display: none;">
        ...
        <div class="mini-cart-footer">
            ...
            <c:if test="${not empty param.returnUrl}">
                <button class="btn-payment" onclick="returnToCustomerInfo()" style="background: #4CAF50;">
                    <i class="fas fa-check-circle"></i> Hoàn tất chọn dịch vụ
                </button>
            </c:if>
            <c:if test="${empty param.returnUrl}">
                <button class="btn-payment" onclick="confirmOfflineServicePayment()">
                    <i class="fas fa-check-circle"></i> Xác nhận thanh toán
                </button>
            </c:if>
        </div>
...
function confirmOfflineServicePayment() {
    if (cart.length === 0) {
    ...
    // Xác nhận thanh toán
    const confirmMsg = 'Xác nhận thanh toán ' + cartTotal.toLocaleString() + '₫ cho ' + cart.length + ' dịch vụ?';
    if (!confirm(confirmMsg)) {
        return;
    }
    
    // Lưu service cart vào server session trước
    const cartData = {
        serviceCart: cart,
        totalAmount: cartTotal
    };
    
    console.log('💾 Đang lưu service cart để in hóa đơn...', cartData);
    
    // Lưu vào session và chuyển đến trang hóa đơn
    ...
    .then(data => {
        console.log('✅ Service cart đã được lưu:', data);
        
        if (data.success) {
            // Chuyển đến trang hóa đơn dịch vụ
            window.location.href = '${pageContext.request.contextPath}/service-invoice';
        } else {
            alert('Lỗi khi lưu dịch vụ: ' + data.message);
        }
    })
    .catch(error => {
        console.error('❌ Lỗi khi lưu service cart:', error);
        alert('Lỗi khi lưu dịch vụ. Vui lòng thử lại!');
    });
}

// Function để quay lại Customer-info với service cart đã lưu
function returnToCustomerInfo() {
    // Lưu service cart vào server session trước
    const cartData = {
        serviceCart: cart,
        totalAmount: cartTotal
    ...
    console.log('💾 Đang lưu service cart vào server session...', cartData);
    ...
    .then(response => response.json())
    .then(data => {
        console.log('✅ Service cart đã được lưu vào session:', data);
        
        if (data.success) {
            // Lấy returnUrl từ URL parameter
            ...
            if (returnUrl) {
                ...
                // Nếu không có returnUrl, quay về room-list
