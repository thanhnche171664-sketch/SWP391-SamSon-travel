...
/**
 * Servlet xử lý các thao tác ảnh dành cho phòng (AJAX)
 * URL: /room/images
 */
@WebServlet("/room/images")
public class RoomImageServlet extends HttpServlet {
    ...
    private void handleDelete(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        try {
            ...
            try {
                String imageUrl = image.getImageUrl(); // e.g., uploads/rooms/xxx.jpg
            ...
            Image primary = imageDAO.getPrimaryImage("room", image.getEntityId());
            if (primary == null) {
                Image first = imageDAO.getFirstImage("room", image.getEntityId());
                if (first != null) {
                    imageDAO.setPrimaryImage(first.getId(), "room", image.getEntityId());
                }
            }
    ...
    private void handleSetPrimary(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        try {
            ...
            if (idParam == null || roomIdParam == null) {
                out.print("{\"success\":false, \"message\":\"Missing parameters\"}");
                return;
            }
            int imageId = Integer.parseInt(idParam);
            int roomId = Integer.parseInt(roomIdParam);
            boolean ok = imageDAO.setPrimaryImage(imageId, "room", roomId);
